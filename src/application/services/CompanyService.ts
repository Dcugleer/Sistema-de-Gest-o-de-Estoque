...
// Service Layer para empresas
export class CompanyService {
  static async createCompany(data: CreateCompanyDTO) {
    return await (await import('@/infrastructure/database/repositories')).CompanyRepository.create(data)
  }
  
  static async getCompanies(page: number = 1, limit: number = 10) {
    return await (await import('@/infrastructure/database/repositories')).CompanyRepository.findAll(page, limit)
  }
  
  static async getCompanyById(id: string) {
    return await (await import('@/infrastructure/database/repositories')).CompanyRepository.findById(id)
  }
}
